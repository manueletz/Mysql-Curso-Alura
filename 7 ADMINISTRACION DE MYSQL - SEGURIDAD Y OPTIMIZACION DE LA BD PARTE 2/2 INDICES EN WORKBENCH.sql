...
-- COST 138676.67 -> 6386376719.43 SIN INDICE
SELECT A.CODIGO_DEL_PRODUCTO, YEAR(FECHA_VENTA) AS ANNO, C.CANTIDAD FROM tabla_de_productos1 A 
INNER JOIN items_facturas1 C ON A.CODIGO_DEL_PRODUCTO = C.CODIGO_DEL_PRODUCTO 
INNER JOIN facturas1 B ON C.NUMERO = B.NUMERO;

-- COST 138676.67 -> 6386376719.43 SIN INDICE
SELECT A.CODIGO_DEL_PRODUCTO, YEAR(FECHA_VENTA) AS ANNO, SUM(C.CANTIDAD) AS CANTIDAD FROM tabla_de_productos1 A
INNER JOIN items_facturas1 C ON A.CODIGO_DEL_PRODUCTO = C.CODIGO_DEL_PRODUCTO 
INNER JOIN facturas1 B ON C.NUMERO = B.NUMERO 
